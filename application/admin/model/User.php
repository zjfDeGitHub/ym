...
//                dump($pid);die();
    ...
    public function agencyC($id =0){
            $userModel = new self;
           $downData = $userModel->where('pid','=',$id)->field('consumption,id')->select();
...
//               dump($downData[$k]['mun']);
    ...
    public function qd($id = 0){
        $userModel = new self;
        $level = $userModel->where('id', '=',$id)->field('member,beans,pid')->find();
        //如果有推荐人，推荐人豆子加一
        if ($level['pid']){
...
//            echo '不是一维数组';
    ...
    public function fxj($id=15,$money=100,$len=2){
        ...
        $ratio = db('ratio')->where('type','=',2)->select();
//        dump($parr);
        foreach ($parr as $k =>$v){
           if ($k == 0){
               //推荐奖励，第一代百分之十， 直接加入青春豆
//               $jl = $money*0.1;
               $jl = $money*$ratio[0]['ratio'];
               ...
               $logRes = model('Beanslog')->insert($logData);
//               dump($logRes);
               if (!$logRes){
                   die('logRes错误1');
               }
...
//               dump($logData);die();
//               $logRes = model('Beanslog')->isUpdate(false)->save($logData2);
